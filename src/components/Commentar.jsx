...
const Comment = memo(({ comment, formatDate, index, isPinned = false }) => (
  <div
    className={`px-4 pt-4 pb-2 rounded-xl border transition-all group hover:shadow-lg hover:-translate-y-0.5 ${
      isPinned ? "bg-gradient-to-r from-indigo-500/10 to-purple-500/10 border-indigo-500/30 hover:bg-gradient-to-r hover:from-indigo-500/15 hover:to-purple-500/15" : "bg-white/5 border-white/10 hover:bg-white/10"
    }`}
  >
    {isPinned && (
    ...
    <div className="flex items-start gap-3">
      {comment.profile_image ? (
        <img src={comment.profile_image} alt={`${comment.user_name}'s profile`} className={`w-10 h-10 rounded-full object-cover border-2 flex-shrink-0  ${isPinned ? "border-indigo-500/50" : "border-indigo-500/30"}`} loading="lazy" />
      ) : (
      ...
      <div className="flex-grow min-w-0">
        <div className="flex items-center justify-between gap-4 mb-2">
          <div className="flex items-center gap-2">
            <h4 className={`font-medium truncate ${isPinned ? "text-[#0077B6]" : "text-[#0077B6]"}`}>{comment.user_name}</h4>
            {isPinned && <span className="px-2 py-0.5 text-xs bg-indigo-500/20 text-[#8338ec] rounded-full">Creator</span>}
          </div>
          <span className="text-xs text-gray-400 whitespace-nowrap">{formatDate(comment.created_at)}</span>
        </div>
        <p className="text-[#0096C7] text-sm break-words leading-relaxed relative bottom-2">{comment.content}</p>
      </div>
    </div>
  </div>
));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-2" data-aos="fade-up" data-aos-duration="1000">
        <label className="block text-sm font-medium text-[#0096C7]">
        ...
        <input
          ...
          className="w-full p-3 rounded-xl bg-white/5 border border-gray-300 text-[#0096C7] placeholder-[#00B4D8] focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all"
          required
        />
      </div>
      ...
      <div className="space-y-2" data-aos="fade-up" data-aos-duration="1200">
        ...
        <textarea
          ...
          className="w-full p-4 rounded-xl bg-white/5 border border-gray-300 text-[#0096C7] placeholder-[#00B4D8] focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all resize-none min-h-[120px]"
          required
        />
      </div>
      ...
      <div className="space-y-2" data-aos="fade-up" data-aos-duration="1400">
        <label className="block text-sm font-medium text-[#0096C7]">
          Profile Photo <span className="text-red-400">(optional)</span>
        </label>
        <div className="flex items-center gap-4 p-4 bg-white/5 border border-white/10 rounded-xl">
          {imagePreview ? (
            ...
            <div className="w-full">
              ...
              <p className="text-center text-[#00B4D8] text-sm mt-2">Max file size: 5MB</p>
            </div>
          )}
        </div>
      </div>
  ...
  const handleCommentSubmit = useCallback(
    async ({ newComment, userName, imageFile }) => {
      ...
      try {
        ...
        const isCommentClean = (text) => {
          const bannedWords = ["bodoh", "goblok", "anjing", "bacot", "gelo", "gila", "bangsat"];
          const lowerText = text.toLowerCase();
          return !bannedWords.some((word) => lowerText.includes(word));
        };

        if (!isCommentClean(newComment)) {
          setError("Tolong tidak menggunakan kata kata kasar");
        ...
        setError("Gagal memasukan pesan. Coba kembali.");
        ...
        <div className="space-y-4 h-[328px] overflow-y-auto overflow-x-hidden custom-scrollbar pt-1 pr-1 " data-aos="fade-up" data-aos-delay="200">
          ...
          {loadingComments ? (
            ...
            comments.map((comment, index) => (
              <motion.div key={comment.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: index * 0.05 }}>
